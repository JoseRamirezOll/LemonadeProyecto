...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        println("${lemonadeState}")
         when (lemonadeState)
         {
             SELECT ->  cambiarEstadoASQ() // cambia el estado SQUEEZE
             SQUEEZE -> cambiarEstadoADR() // cambia el estado a DRINK
             DRINK -> cambiarEstadoARE() // cambia el estado a RESTART
             RESTART -> cambiarEstadoASEL() //cambia el estado a SELECT
         }
        /*   if(lemonadeState == SELECT)
            {
                ...
                println(" el valor es aleatoreo es  ${lemonSize}")
            }
    ...
    */
    } // fin del metodo clickLemonImage
    ...
    fun cambiarEstadoADR(){
        // cambia el estado a drink
        setViewElements()
        if(lemonSize == 0)
        {
            lemonadeState = DRINK
    ...
    fun cambiarEstadoARE(){
        // camvvia el estado a restart
        setViewElements()
        lemonSize = -1
        squeezeCount = 0
        lemonadeState =  RESTART
    }
    fun cambiarEstadoASQ(){
        // camvia el estado a squeeze
        setViewElements()
        lemonSize = LemonTree().pick()
        lemonadeState = SQUEEZE
        println(" el valor es aleatoreo es  ${lemonSize}")
    }
    fun cambiarEstadoASEL(){
        // camvia ek estado a select
        setViewElements()
        lemonadeState = SELECT
    }
}
